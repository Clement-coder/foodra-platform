...
function ProductModal({ product, farmer, privyId, onClose, onRefresh, onNotify }: {
  ...
  const toggle = async () => {
    ...
    onNotify(product.is_available ? "Product deactivated" : "Product activated")
    onRefresh(); onClose()
  }
  ...
  const remove = async () => {
    ...
    setLoading(true)
    await fetch(`/api/products/${product.id}?actorPrivyId=${privyId}`, { method: "DELETE" })
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl flex items-center gap-2">
              ...
              <div><p className="text-xs text-gray-400">Location</p><p className="text-sm font-medium">{product.location || "—"}</p></div>
            </div>
            <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl flex items-center gap-2">
              <Calendar className="w-4 h-4 text-gray-400 flex-shrink-0" />
              <div><p className="text-xs text-gray-400">Listed</p><p className="text-sm font-medium">{new Date(product.created_at).toLocaleDateString()}</p></div>
            </div>
          </div>
          {farmer && (
            <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
              ...
              <div className="flex items-center gap-3">
                {farmer.avatar_url
                  ...
                  : <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold">{(farmer.name || "F")[0].toUpperCase()}</div>
                }
                <div>
          ...
          <div className="flex gap-2 pt-2">
            <button onClick={toggle} disabled={loading}
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 ${product.is_available ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-200" : "bg-green-100 text-green-700 hover:bg-green-200"}`}>
            ...
            <button onClick={remove} disabled={loading}
              className="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-sm font-medium bg-red-100 text-red-700 hover:bg-red-200 transition-colors disabled:opacity-50">
...
export default function AdminProducts({ data, privyId, onRefresh, onNotify }: {
  ...
  return (
    <>
      {selected && <ProductModal product={selected} farmer={getFarmer(selected.farmer_id)} privyId={privyId} onClose={() => setSelected(null)} onRefresh={onRefresh} onNotify={onNotify} />}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {data.products.map((p: any) => (
              <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    {p.image_url ? <img src={p.image_url} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
                      : <div className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center flex-shrink-0"><ImageIcon className="w-4 h-4 text-gray-400" /></div>}
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${p.is_available ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                    {p.is_available ? "Active" : "Inactive"}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <button onClick={() => setSelected(p)} className="text-xs bg-gray-100 dark:bg-gray-800 hover:bg-green-100 dark:hover:bg-green-900/30 text-gray-700 hover:text-green-700 px-3 py-1.5 rounded-lg transition-colors font-medium">View</button>
                </td>
              </tr>
            ))}
          </tbody>
