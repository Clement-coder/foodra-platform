...
export async function GET(request: Request) {
  try {
    ...
    const [
      users, products, funding, orders,
      enrollments, supportMessages, trainings,
      walletRequests, disputes,
      walletTx, walletAccounts, paystackPayments,
    ] = await Promise.all([
      ...
      supabaseAdmin.from("orders").select("id, buyer_id, total_amount, status, wallet_paid, paid_at, delivery_full_name, delivery_address, delivery_city, delivery_state, delivery_phone, created_at, order_items(id, product_id, product_name, quantity, price, image_url)").order("created_at", { ascending: false }),
      ...
      supabaseAdmin.from("wallet_transactions").select("id, user_id, type, category, amount_ngn, balance_after, reference, note, created_at").order("created_at", { ascending: false }).limit(2000),
      supabaseAdmin.from("wallet_accounts").select("id, user_id, balance_ngn, foodra_tag, created_at"),
      supabaseAdmin.from("paystack_payments").select("id, user_id, amount_ngn, status, created_at").order("created_at", { ascending: false }).limit(1000),
    ])
    ...
    return NextResponse.json({
      ...
      walletAccounts: walletAccounts.data || [],
      paystackPayments: paystackPayments.data || [],
    })
  } catch (error) {
