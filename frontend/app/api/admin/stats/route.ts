...
export async function GET(request: Request) {
  try {
    ...
    const [users, products, funding, orders, enrollments, supportMessages, trainings, walletRequests, disputes, verificationRequests] = await Promise.all([
      ...
      supabaseAdmin.from("orders").select("id, buyer_id, total_amount, status, escrow_status, escrow_tx_hash, delivery_full_name, delivery_address, delivery_city, delivery_state, delivery_phone, created_at, order_items(id, product_id, product_name, quantity, price, image_url, escrow_order_id, farmer_wallet)").order("created_at", { ascending: false }),
