...
import { getPositions, predictNextPrice } from "@/lib/assetStore"
import { META, DEFAULT_META, PriceChart, getChange, CommodityDetailModal } from "./components"
import { MarketPricesPageSkeleton, CommodityCardSkeleton } from "@/components/Skeleton"

// ─── Commodity icon (Lucide, no emoji) ────────────────────────────────────────
export function CommodityIcon({ commodity, className, style }: { commodity: string; className?: string; style?: React.CSSProperties }) {
  ...
  if (n.includes("meat") || n.includes("beef"))
    return <Drumstick className={className} style={style} />
  ...
  return <ShoppingBasket className={className} style={style} />
}
...
function CommodityCard({ item, history, index, onClick }: {
  ...
  return (
    ...
    >
      {/* Chart header */}
      <div className="relative h-28" style={{ background: `linear-gradient(160deg, ${meta.bg}cc, ${meta.bg}40)` }}>
        ...
        <div className="absolute top-2.5 inset-x-3 flex items-start justify-between">
          ...
          {/* Change badge */}
          {change ? (
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-lg flex items-center gap-0.5 backdrop-blur-sm
            ...
            <span className="text-[10px] bg-black/10 text-muted-foreground px-1.5 py-0.5 rounded-lg">—</span>
          )}
        </div>
      </div>
      ...
      <div className="p-3 space-y-2.5">
        <div>
          ...
          <p className="text-xs text-muted-foreground line-clamp-1 mt-0.5">{meta.desc}</p>
        </div>
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            {item.usdPrice > 0 && <p className="text-[10px] text-muted-foreground">≈ ${item.usdPrice.toFixed(2)} USD</p>}
          </div>
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            <MapPin className="h-2.5 w-2.5" />
            <span>{item.marketCount} mkts</span>
          </div>
        </div>

        {/* AI Trend estimate */}
        {prediction !== null && (
          <div className="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/40 rounded-xl px-2.5 py-1.5">
            <BrainCircuit className="h-3 w-3 text-indigo-500 shrink-0" />
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 flex items-center gap-1">
                AI Trend Estimate <LineChart className="h-2.5 w-2.5" />
              </p>
              <p className="text-[10px] text-indigo-500 dark:text-indigo-400">
                ₦{prediction.toLocaleString()} next period
              </p>
            </div>
          </div>
        )}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {held != null && (
            <span className="text-[10px] bg-[#118C4C]/15 text-[#118C4C] rounded-full px-1.5 py-0.5 font-medium shrink-0">
              {held} {item.unit}
            </span>
          )}
        </div>
        {prediction !== null && (
          <p className="text-[10px] text-indigo-500 dark:text-indigo-400 mt-0.5 flex items-center gap-1">
            ...
            Next: ₦{prediction.toLocaleString()}
          </p>
        )}
      </div>
      ...
      <div className="text-right shrink-0 min-w-[90px]">
        <div className="flex items-baseline gap-0.5 justify-end">
          ...
          <span className="text-[9px] text-muted-foreground font-normal">/{item.unit}</span>
        </div>
        {change ? (
          <p className={`text-[10px] font-semibold flex items-center justify-end gap-0.5 mt-0.5
...
export default function MarketPricesPage() {
  ...
  const SORT_OPTIONS = [
    { value: "name",   label: "A – Z",           icon: ArrowUpDown,  desc: "Alphabetical order" },
    { value: "price",  label: "Highest Price",    icon: TrendingDown, desc: "Most expensive first" },
    { value: "change", label: "Biggest Movers",   icon: TrendingUp,   desc: "Largest % change" },
  ] as const

  // Show full-page skeleton on first load
  if (loading && prices.length === 0) return <MarketPricesPageSkeleton />
          ...
          <div className="relative overflow-hidden border-b border-white/10 bg-black/20">
            ...
            >
              {[...withChange, ...withChange].map((p, i) => (
                <span key={i} className="flex items-center gap-1.5 px-4 py-1.5 border-r border-white/10 shrink-0 text-[11px] font-mono">
                  <CommodityIcon commodity={p.commodity} className="h-3 w-3 text-white/50 shrink-0" />
                  <span className="font-semibold text-white/85">{p.displayName}</span>
                  <span className="text-white font-bold">
                    ₦{p.price.toLocaleString()}
                    <span className="text-white/40 font-normal text-[9px] ml-0.5">/{p.unit}</span>
                  </span>
                  {p.change && (
        ...
        <div className="relative container mx-auto px-4 py-8 max-w-5xl">
          ...
          {!loading && prices.length > 0 && (
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Commodities",  value: prices.length, sub: "tracked",        Icon: ShoppingBasket },
                { label: "Gainers",      value: gainers,       sub: "this period",    Icon: TrendingUp },
                { label: "Losers",       value: losers,        sub: "this period",    Icon: TrendingDown },
                { label: "Avg Markets",  value: Math.round(prices.reduce((s, p) => s + p.marketCount, 0) / prices.length), sub: "per commodity", Icon: MapPin },
              ].map(s => (
        ...
        <div className="container mx-auto px-4 py-3 max-w-5xl flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-border bg-background px-3 py-2 text-sm font-medium hover:bg-muted transition-colors min-w-[150px] justify-between"
            >
              <div className="flex items-center gap-2">
                {(() => {
                  const opt = SORT_OPTIONS.find(o => o.value === sortBy)!
                  return <opt.icon className="h-3.5 w-3.5 text-muted-foreground" />
                })()}
              ...
              {sortOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 z-20 w-56 rounded-2xl border border-border bg-card shadow-2xl overflow-hidden"
                  >
                    <div className="px-3 pt-3 pb-1.5">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Sort by</p>
                    </div>
                    {SORT_OPTIONS.map(opt => (
                      <button key={opt.value}
                        onClick={() => { setSortBy(opt.value); setSortOpen(false) }}
                        className={`w-full flex items-start gap-3 px-4 py-3 text-sm transition-colors hover:bg-muted
                          ${sortBy === opt.value ? "text-[#118C4C] bg-[#118C4C]/5" : "text-foreground"}`}
                      >
                        <opt.icon className={`h-4 w-4 shrink-0 mt-0.5 ${sortBy === opt.value ? "text-[#118C4C]" : "text-muted-foreground"}`} />
                        <div className="flex-1 text-left">
                          <p className={`font-semibold leading-tight ${sortBy === opt.value ? "text-[#118C4C]" : ""}`}>{opt.label}</p>
                          <p className="text-[10px] text-muted-foreground mt-0.5">{opt.desc}</p>
                        </div>
                        {sortBy === opt.value && <Check className="h-4 w-4 shrink-0 mt-0.5 text-[#118C4C]" />}
                      </button>
                    ))}
                  </motion.div>
                </>
      ...
      <div className="container mx-auto px-4 py-6 max-w-5xl">
        {loading ? (
            ...
            <div className="rounded-2xl border border-border overflow-hidden bg-card divide-y divide-border">
              {Array.from({ length: 10 }).map((_, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3.5">
                  <div className="w-9 h-9 rounded-xl bg-muted animate-pulse shrink-0" style={{ animationDelay: `${i*60}ms` }} />
                  <div className="flex-1 space-y-1.5">
                    <div className="h-3.5 w-36 rounded-lg bg-muted animate-pulse" style={{ animationDelay: `${i*60+30}ms` }} />
                    <div className="h-2.5 w-24 rounded bg-muted animate-pulse" style={{ animationDelay: `${i*60+60}ms` }} />
                  </div>
                  <div className="hidden sm:block w-16 h-8 rounded-lg bg-muted animate-pulse" />
                  <div className="text-right space-y-1">
                    <div className="h-3.5 w-20 rounded-lg bg-muted animate-pulse ml-auto" />
          ...
          <div className="text-center py-20 text-muted-foreground">
            <Wheat className="h-10 w-10 mx-auto mb-3 opacity-20" />
          ...
          <div className="rounded-2xl border border-border overflow-hidden bg-card">
            <div className="px-4 py-2.5 border-b border-border bg-muted/40 grid grid-cols-[2.25rem_1fr_5rem_4rem_5.5rem] gap-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">
              <span />
              <span>Commodity</span>
              <span className="hidden sm:block text-center">AI Next</span>
